...
const SkillBadge = (skills: { name: string; image: string }[]) => {
    return skills.map((skill, index) => (
        <div key={index} className="flex gap-2 border border-textColor rounded-2xl items-center py-2 px-3 bs-mx:py-0 bs-mx:px-1.5 bs-mx:gap-1 mb-1">
            <img className="w-[48px] bs-mx:w-[36px] xsm-mx:w-[28px] !p-1" src={skill.image} alt={skill.name} />
            <div className="text-textColor text-xl font-medium sm-mx:text-lg xs-mx:text-sm">{skill.name}</div>
        </div>
    ));
};
